...
public class Utilities {
    ...
    public String tDID, tCID, tSID, firstName, lastName, execute, browser, url, email, password, welcomeMessage, deptCity, destCity, deptDate, retDate, dayTime, resultTestData, resultTestStep, errorAt =" ", errorReport, keyword,	input,	path, tError;
    ...
    public static WebDriver getDriver(String browser) {
        ...
        if (browser.equalsIgnoreCase("ie") || browser.equalsIgnoreCase("iexplorer") || browser.equalsIgnoreCase("internet explorer")) {
            return driver = new ChromeDriver();
        }
        if (browser.equalsIgnoreCase("htmlunitdriver")) {
            return driver = new HtmlUnitDriver();
        } else return driver = new FirefoxDriver();
    }
...
//        return input;
    ...
    public static String [][] readXL(String sPath, String iSheet) throws Exception{
        ...
        if(sPath.toLowerCase().endsWith("xlsx")){
            ...
            File myxl1 = new File(sPath); // first create a file object providing the path to that file
            FileInputStream myStream = new FileInputStream(myxl1); // create input stream object using the file object created above

            HSSFWorkbook myWB = new HSSFWorkbook(myStream); // WorkBook is an Apache plugin the connects and reads from Excel
            HSSFSheet mySheet = myWB.getSheet(iSheet); // create object of a  specific sheet from the Excel file
